...
class DQNBreakout(gym.Wrapper):
    def __init__(self, render_mode='rgb_array', repeat=4, device='cpu'):
        env = gym.make("BreakoutNoFrameskip-v4", render_mode=render_mode)  # Corrected environment ID
    ...
    def step(self, action):
        ...
        for i in range(self.repeat):
            observation, reward, done, info = self.env.step(action)  # Pass action to step
